...
class FastAccess(QWidget):
    def __init__(self, parent=None):
        ...
        self.lines = QVBoxLayout()               # The Vertical Box that contains the Horizontal Boxes of  labels and buttons
        self.lines.setSpacing(self.height()/50)
        self.lines.setContentsMargins(self.width()/60,self.height()/50,self.width()/50,self.height()/50)
    ...
    def add_button(self,name, adress):
        ...
        delete_btn.clicked.connect(lambda: self.delete(name.text()))
        ...
        self.lines.insertWidget(0, line)  # Добавление в начало, с пружиной это прижмет вверх
...
class SettingsWindow(QMainWindow):
    ...
    def init(self):
        ...
        self.AppearenceBttn = QPushButton("Быстрый доступ",parent = self) #местами поменять надо, случайно изначально их перепутал
        ...
        self.stacklayout.addWidget(FastAccess())
    ...
    def tabs(self):
        self.AppearenceBttn.setMinimumSize(self.width()/2-self.height()/4,self.height()/8)
        self.FastAccessBttn.setMinimumSize(self.width()/2-self.height()/4,self.height()/8)
        ...
        self.ExitBttn.setStyleSheet(self.close_btn + " border-top-left-radius: 25%;")
